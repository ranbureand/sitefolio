...
excerpt: The include `language-switch.html` generates an unordered list containing all the languages supported in the site […].
...
In the code above, we loop through the languages defined in the `snippets.html` file (read the section [Snippets]({% post_url 2021-08-13-multilingual-sites-in-jekyll-2 %}#snippets) for more details).
